...
function showActionModal(rfidCode, studentName, studentId, isActivated, existingRfid) {
    ...
    $('#assignRfidBtn').data('student-id', studentId);

    // Reset button states
    $('#activateBtn').removeClass('disabled-btn').attr('disabled', false);
    $('#deactivateBtn').removeClass('disabled-btn').attr('disabled', false);
    $('#assignRfidBtn').removeClass('disabled-btn').attr('disabled', false);
    $('#assignNewRfidBtn').removeClass('disabled-btn').attr('disabled', false);

    // Check if the student already has an RFID assigned
    if (existingRfid) {
        ...
        if (isActivated === 1) {
            ...
            $('#assignNewRfidBtn').removeClass('disabled-btn').attr('disabled', true); // Enable Assign New RFID button
        } else {
        ...
        // If there is no existing RFID, handle based on activation status
        if (isActivated === 1) { // Activated
            $('#activateBtn').addClass('disabled-btn').attr('disabled', true); // Disable Activate button
            $('#deactivateBtn').removeClass('disabled-btn').attr('disabled', false); // Enable Deactivate button
            $('#assignRfidBtn').removeClass('disabled-btn').attr('disabled', false); // Enable Assign RFID button
            $('#assignNewRfidBtn').removeClass('disabled-btn').attr('disabled', false); // Enable Assign New RFID button
        } else if (isActivated === 0) { // Disabled
            $('#activateBtn').removeClass('disabled-btn').attr('disabled', true); // Enable Activate button
            $('#deactivateBtn').addClass('disabled-btn').attr('disabled', true); // Disable Deactivate button
            $('#assignRfidBtn').removeClass('disabled-btn').attr('disabled', false); // Enable Assign RFID button
            $('#assignNewRfidBtn').addClass('disabled-btn').attr('disabled', true); // Disable Assign New RFID button
        }
